...
Grades.getCoursesWithStudents = function getCoursesWithStudents() {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.professorCoursesWithEnrolledStudents.length === 0) {
                $(warning).find('h1').text("You have't added any course yet!");
                warning.show();
            } else {
                Utils.sortCourseAscending(data.professorCoursesWithEnrolledStudents);
                Grades.renderCourses(data.professorCoursesWithEnrolledStudents);
    ...
    $.ajax({
        ...
        success: function (data) {
            if(data.enrolledStudents.length === 0) {
                ...
                warning2.hide();
                Utils.sortStudentsAscending(data.enrolledStudents);
                Grades.renderStudents(data.enrolledStudents);
            }
        },
        error: function (data) {
            console.log(data);
