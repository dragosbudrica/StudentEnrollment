...
ProfessorCourses.getProfessorCourses = function getProfessorCourses() {
    ...
    $.ajax({
        ...
        success: function (data) {
            if(data.professorCourses.length === 0) {
                $(warning).find('h1').text("You have't added any course yet!");
                warning.show();
            } else {
                Utils.sortCourseAscending(data.professorCourses);
                ProfessorCourses.renderCourses(data.professorCourses);
...
ProfessorCourses.getAllCategories = function getAllCategories(editButton) {
    $.ajax({
        ...
        traditional: true,
        success: function (data) {
            for (index = 0; index < data.allCategories.length; ++index) {
                var category = "<option value=" + data.allCategories[index].categoryName + ">" + data.allCategories[index].categoryName + "</option>";
